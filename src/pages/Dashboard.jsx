...
const COLORES = ['#3FC6D6', '#F2A93B', '#4ADE80', '#F0665F', '#8B96A6', '#9D7BEA', '#5AC8E0'];
  ...
  return (
    <Layout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        <Tarjeta label="Materiales activos" valor={resumen?.total ?? '—'} color="var(--cyan)" />
        <Tarjeta label="Bajas registradas" valor={resumen?.bajas ?? '—'} color="var(--red)" />
        <Tarjeta label="Sin movimiento" valor={sinMovimiento?.materiales?.length ?? '—'} color="var(--amber)"
      ...
      {sinMovimiento?.materiales?.length > 0 && (
        <div className="card" style={{ marginBottom: 24, borderColor: 'rgba(242,169,59,0.4)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <h3 style={{ fontSize: 15, color: 'var(--amber)' }}>Materiales sin movimiento reciente</h3>
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={resumen?.porClasificacion || []} layout="vertical" margin={{ left: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2C3644" horizontal={false} />
              <XAxis type="number" stroke="#8B96A6" fontSize={12} />
              <YAxis type="category" dataKey="nombre" stroke="#8B96A6" fontSize={12} width={110} />
              <Tooltip contentStyle={{ background: '#1A2029', border: '1px solid #2C3644', borderRadius: 8 }} />
              <Bar dataKey="total" fill="#3FC6D6" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="card">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={resumen?.altasPorMes || []}>
            <CartesianGrid strokeDasharray="3 3" stroke="#2C3644" vertical={false} />
            <XAxis dataKey="mes" stroke="#8B96A6" fontSize={12} />
            <YAxis stroke="#8B96A6" fontSize={12} allowDecimals={false} />
            <Tooltip contentStyle={{ background: '#1A2029', border: '1px solid #2C3644', borderRadius: 8 }} />
